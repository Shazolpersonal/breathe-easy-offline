...
## 2026-05-04 - O(N) Aggregation in React Components\n**Learning:** Chaining multiple array methods like `.filter()`, `.reduce()`, `.map()`, and `.forEach()` inside `useMemo` creates unnecessary intermediate arrays and multiple passes over the same dataset, causing performance and GC overhead.\n**Action:** Refactor chained array iterations into a single `for...of` or `for` loop to compute all needed metrics simultaneously in one pass, especially for components that re-render frequently or process large datasets.
