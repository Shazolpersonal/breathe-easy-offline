...
## 2025-02-28 - Optimize Consistency Score Calculation\n**Learning:** Chained array methods like `.map().reduce()` inside functions called frequently or dynamically computed over large datasets like `sessions` force multiple O(N) passes through the array and create intermediate memory garbage.\n**Action:** Replaced chained higher-order array methods with a single `for...of` loop in `src/lib/consistency.ts` to compute consistency variables directly, significantly avoiding multiple iterations over array data.
