...
export default function Programs() {
      ...
      <div className="min-h-screen px-4 pb-24 pt-12">
        <div className="mx-auto max-w-md">
          ...
          <div className="mb-6">
            ...
            <h1 className="mt-2 text-2xl font-bold text-foreground">{program.name}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{program.description}</p>
            <p className="mt-2 text-xs text-muted-foreground">
              {completedDays.length}/{program.days.length} days completed
            </p>
          </div>
          ...
          <div className="space-y-2">
            {program.days.map((day) => {
              const technique = getTechniqueById(day.techniqueId, getCustomTechniques());
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isNext && <span className="rounded-full bg-primary/20 px-2 py-0.5 text-[10px] font-medium text-primary">Next</span>}
                      </div>
                      <p className="text-xs text-muted-foreground">{technique?.name || day.techniqueId} · {day.durationMinutes} min</p>
                      <p className="mt-1 text-xs text-muted-foreground/80 italic">{day.tip}</p>
                    </div>
  ...
  return (
    <div className="min-h-screen px-4 pb-24 pt-12">
      <div className="mx-auto max-w-md">
        <h1 className="mb-6 text-2xl font-bold text-foreground">Guided Programs</h1>

        <div className="space-y-3">
          {PROGRAMS.map((p) => {
            const e = enrollments.find(en => en.programId === p.id);
            const progress = e ? `${e.completedDays.length}/${p.days.length} days` : `${p.days.length} days`;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      {e && (
                        <span className="rounded-full bg-primary/20 px-2 py-0.5 text-[10px] font-medium text-primary">Enrolled</span>
                      )}
                    </div>
                  </div>
